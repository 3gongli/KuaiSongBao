...
public class Song_MyOrder_NCactivity extends ListActivity {
	...
	// public Context mContext = Estake_activity.this;
	...
	void init() {
		...
		mPullRefreshListView.setBackgroundColor(getResources().getColor(
				R.color.white));
		// sp = Song_MyOrder_NCactivity.this.getSharedPreferences("registered",
		// MODE_PRIVATE);
				...
				public void run() {
					try {
						if (!isRefresh) {
							...
							params.add(new BasicNameValuePair("status", "0")); // 未取
							obj = new JSONObject(http.httppost(
									HttpPathUtils.query_sp_waybill, params));
							Log.i("----------->obj", obj.toString() + "..");
							if (obj.getString("success").equals("true")) {
								Song_MyOrderActivity.ncTotalNum = obj
								...
								msg.what = Song_MyOrderActivity.MSG_GET_TAKE_LIST_OK;
								handler.sendMessage(msg);

							} else {
								Message msg = new Message();
								msg.what = Song_MyOrderActivity.MSG_GET_TAKE_LIST_FAILED;
								handler.sendMessage(msg);
								isRefresh = false;
							}
						}
	...
	public static OrderList getMap(JSONObject oneDataObj) throws JSONException {
		...
		// Object idObj = oneDataObj.get("id");
		// if(idObj!=null){
		// idObj.toString();
		// }else {
		// id="";
		...
		takeOrderData.setName_phone(oneDataObj.getString("courier_real_name")
				+ " / " + oneDataObj.getString("courier_phone"));
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case Song_MyOrderActivity.MSG_GET_TAKE_LIST_OK:
				Log.i("Es_activity", "MSG_GET_TAKE_LIST_OK");
				Toast.makeText(Song_MyOrder_NCactivity.this, "刷新待取运单列表成功", 0)
						.show();
				...
				OrderListAdapter adapter = new OrderListAdapter(list,
						Song_MyOrder_NCactivity.this, 0);
				list.clear();
				...
				break;
			case Song_MyOrderActivity.MSG_GET_TAKE_LIST_FAILED:
				// Log.i("Es_activity", "MSG_GET_TAKE_LIST_FAILED");
				Toast.makeText(Song_MyOrder_NCactivity.this, "刷新待取运单列表失败", 0)
						.show();
