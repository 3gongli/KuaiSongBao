...
public class Song_MyOrder_Deliveryactivity extends ListActivity {
	private PullToRefreshListView mPullRefreshListView;
	public boolean isRefresh = false;
	private SharePrefUtil sp = new SharePrefUtil();
	private JSONObject obj;
	private List<OrderList> list = new ArrayList<OrderList>();
	public static final String ACTION_REFRESH_DELIVERY_LISTVIEW = "action_refresh_delivery_listview";
				...
				public void run() {
					try {
						if (!isRefresh) {
							...
							params.add(new BasicNameValuePair("status", "1")); // 配送中
							obj = new JSONObject(http.httppost(
									HttpPathUtils.query_sp_waybill, params));
							Log.i("----------->obj", obj.toString() + "..");
							if (obj.getString("success").equals("true")) {
								Song_MyOrderActivity.deliveryTotalNum = obj
								...
								msg.what = Song_MyOrderActivity.MSG_GET_DELIVERY_LIST_OK;
								handler.sendMessage(msg);

							} else {
								Message msg = new Message();
								msg.what = Song_MyOrderActivity.MSG_GET_DELIVERY_LIST_FAILED;
								handler.sendMessage(msg);
								isRefresh = false;
							}
						}
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case Song_MyOrderActivity.MSG_GET_DELIVERY_LIST_OK:
				Toast.makeText(Song_MyOrder_Deliveryactivity.this,
						"刷新配送中运单列表成功", 0).show();
				...
				OrderListAdapter adapter = new OrderListAdapter(list,
						Song_MyOrder_Deliveryactivity.this, 1);
				list.clear();
				...
				break;
			case Song_MyOrderActivity.MSG_GET_DELIVERY_LIST_FAILED:
	...
	private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (action.equals(ACTION_REFRESH_DELIVERY_LISTVIEW)) {
				new GetDataTask().execute();
			}
		}
	...
	public void registerBoradcastReceiver() {
		...
		myIntentFilter.addAction(ACTION_REFRESH_DELIVERY_LISTVIEW);
		// 注册广播
